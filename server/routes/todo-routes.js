...
module.exports = {
	...
	createTodo : function(req, res) {
		...
		console.log("data"+data);
  		...
  		pg.connect(conString, function(err, client, done) {
   			client.query("INSERT INTO todos(text, done,id_user) values($1, $2, $3)", [data.text, data.done,data.id_user]);

			var query = client.query("SELECT * FROM todos WHERE id_user=($1) ORDER BY id ASC",[data.id_user]);
	...
	getTodos : function(req, res) {
		...
		// get a pg client from the connection pool
  		pg.connect(conString, function(err, client, done) {
			...
			var query = client.query("select t.* from users u, todos t where u.id=t.id_user and u.username=($1) and u.password=($2) ; ",[data.username,data.password]);
	...
	=================================================================*/
	//Get all todos in the database
	getTodosIdUser : function(req, res) {
		results = [];
		var data={
			"id_user":req.body.id_user
		};
